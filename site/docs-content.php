...
return array(
	...
	'debugging' => array(
		...
		'sections' => array(
			...
			array( 'p', 'Two views live on every site\'s tools page. `https://SLUG.test/.agent-local/errors` is the error list, grouped and counted, over 15m, 1h, 24h or 7d. `/.agent-local/requests` is what the site served: method, path, status, how long it took, how big the response was, whether it came from disk or PHP or a redirect or the media fallback, and any PHP error that request logged. The pool log holds the errors but not the requests. The browser holds the requests but not the errors. This page holds both.' ),
			array( 'p', 'Rows arrive as the site serves them, so the page can sit open beside the site. Pause stops the feed and counts what arrives while it is stopped. Click a request for the exchange: URL, status, timing, the PHP file that answered, response headers, request headers, and its errors. `Cookie` and `Authorization` are never recorded.' ),
			array( 'pre', "clear_requests            # empty the log\n# reproduce with a browser tool, or http_request\nget_requests errors_only  # the failing request, with its errors attached" ),
			array( 'note', 'The newest 2048 requests are kept in memory, so a restart starts a fresh log. Errors are the lines the pool log gained while a request ran, so two requests at once can show the same line. Only the built-in router records; under the Apache front the page says so.' ),
			array( 'h', 'WP_DEBUG' ),
	...
	'agents' => array(
		...
		'sections' => array(
			...
			array( 'table', array( 'Area', 'Tools' ), array(
				...
				array( 'lifecycle', '`create_site`, `attach_site`, `import_site`, `start_site`, `stop_site`, `restart_site`, `move_site`, `delete_site`' ),
